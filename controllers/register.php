<?php include('config/db.php');
    ...
    if(isset($_POST["submit"])) {
        ...
        $email_check_query = mysqli_query($connection, "SELECT * FROM phpusers WHERE email = '{$email}' ");
            ...
            if($rowCount > 0) {
                ...
                if((preg_match("/^[a-zA-Z ]{2,10}$/", $_first_name)) && (preg_match("/^[a-zA-Z ]{2,10}$/", $_last_name)) &&
                 ...
                 (preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{8,20}$/", $_password))){
                    ...
                    // Query
                    $sql = "INSERT INTO phpusers (firstname, lastname, email, mobilenumber, password, token, is_active,
                    ...
                    if($sqlQuery) {
						...
						// Note: This personal information will needed to be changes before submitting to GitHub
                        //Should be moved to the config folder together with db info !!					
                       //--------------------------------------Swiftmail GitHub---------------------------
